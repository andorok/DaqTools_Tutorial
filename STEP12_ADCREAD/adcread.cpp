...
BRDCHAR g_AdcSrvName[64] = _BRDC("ADC112x2G6DA0"); // с номером службы
...
// подготовить АЦП к началу сбора (корректировка параметров АЦП, связанная с использование интерфейса JESD204B)
...
// выполнить сбор данных из АЦП в ПК
...
int BRDC_main( int argc, BRDCHAR *argv[] )
{
	...
	int use_mem = 1; // 1 - использовать динамическую память в качестве FIFO, 0 - не использовать
	ADC_PARAM adc_param;
	dev_info.size = sizeof(ADC_PARAM);
	S32 ret = ADC_set(hADC, idev, g_AdcSrvName, _BRDC("adc.ini"), &adc_param, use_mem);
	if (ret != -1)
	{
		...
		ret = ADC_allocbuf(hADC, &bBufSize);
		if (ret == -1)
		...
		else
		{
			if (BRD_errcmp(ret, BRDerr_OK))
			{
				...
				//====================================================================================
				ret = ADC_prepare(hADC);
				// выполнить сбор данных с АЦП
				BRDC_printf(_BRDC("ADC is starting...     \r"));
				ret = ADC_read(hADC, use_mem);

				if (BRD_errcmp(ret, BRDerr_OK))
				{
					BRDC_printf(_BRDC("DAQ by DMA from FIFO (SDRAM) is complete!!!\n"));
...
// проверяем установлена ли динамическая память на устройстве и
// конфигурируем ее в качестве FIFO для АЦП
...
// memflag - (IN) флаг использования динамической памяти в качестве FIFO 
int ADC_set(BRD_Handle hADC, int iDev, BRDCHAR* adcsrv, BRDCHAR* inifile, ADC_PARAM* adcpar, int memflag)
{
	S32		status;
