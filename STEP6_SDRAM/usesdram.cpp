		...
		if(handle[iDev] > 0)
		{
			...
			if(ItemReal <= MAX_SRV)
			{
				...
				if(hMem > 0 && hADC > 0)
				{
					...
					status = DaqIntoSdramDMA(hADC, hMem);
					if (BRD_errcmp(status, BRDerr_OK))
					{
						IPC_handle hfile = 0;
					...
					{
						BRDC_printf(_BRDC("Daq data is NOT complete (0x%08X) - timeout 1 sec\n"), status);
					}
...
// выполнить сбор данных в SDRAM с опросом флага окончания сбора
// (без использования прерывания по окончанию сбора)
// и подготовить стрим к передаче данных в ПК
S32 DaqIntoSdramDMA(BRD_Handle hADC, BRD_Handle hMem)
{
	...
	ULONG msTimeout = 10000; // ждать окончания сбора данных в память до 10 сек.
	ULONG cntTimeout = msTimeout / 100;
	ULONG evt_status = BRDerr_WAIT_TIMEOUT;
	// дожидаемся окончания сбора
	for (ULONG i = 0; i < cntTimeout; i++)
	{
		status = BRD_ctrl(hMem, 0, BRDctrl_SDRAM_ISACQCOMPLETE, &isAcqComplete);
		if (isAcqComplete)
		{
...
// получить данные из памяти в ПК
S32 DataFromMem(BRD_Handle hADC)
{
	S32		status;
